# FeaturesSection Redesign Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Fully replace the scroll-driven sidebar-nav + FeatureBlock architecture with a horizontal tab bar + browser-framed video panel layout.

**Architecture:** The entire `FeaturesSection.tsx` is replaced in one task — the new component is self-contained with no sub-component imports beyond `VideoPlayer` and `VideoModal`. The old `FeatureBlock` sub-component (lines 530–688) and `FeatureGrid` import are deleted. Data that was inline JSX is extracted into module-level arrays. State goes from `activeFeature: string` + IntersectionObserver to a simple `activeTab: number`.

**Tech Stack:** Framer Motion (`motion`, `AnimatePresence`, `LayoutGroup`, `layoutId`), Tailwind CSS, TypeScript, React. `VideoPlayer` and `VideoModal` are preserved. No new dependencies.

---

## Task 1: Rewrite FeaturesSection.tsx

**Files:**
- Modify: `apps/marketing/components/sections/FeaturesSection.tsx`

This is a full file replacement. Read the current file first to understand what's being removed, then write the new version.

**Step 1: Read the current file**

Run: `head -20 apps/marketing/components/sections/FeaturesSection.tsx`
(Just to confirm line count and imports before replacing.)

**Step 2: Write the complete new file**

Replace the entire contents of `apps/marketing/components/sections/FeaturesSection.tsx` with:
                        ...
                        <div className="flex gap-2 overflow-x-auto scrollbar-none pb-1">
                            {tier1Features.map((f, i) => (
                                <button
                                    ...
                                    className={`relative flex items-center gap-1.5 px-5 py-2.5 rounded-full text-sm font-medium whitespace-nowrap flex-shrink-0 transition-colors ${
                                        activeTab === i
                                            ? 'text-white'
...
If you see an error about `bg-primary/15` (Tailwind JIT arbitrary opacity) — that's a CSS class, not TypeScript. Ignore it; TypeScript won't flag Tailwind classes.

If you see errors about missing `cn` or `LayoutGroup` — double-check the imports at the top of the file match the new file exactly.

**Step 4: Commit**

```bash
git add apps/marketing/components/sections/FeaturesSection.tsx
git commit -m "feat: features section - full redesign with horizontal tabs and browser-framed video"
```
...
Open `http://localhost:3000` in the browser and scroll to the Features section.

**Step 2: Verify the section header**

- [ ] Eyebrow pill `"The Platform"` appears in primary blue with rounded-full
- [ ] H2 `"Built for how print shops actually work"` is large, bold, centered
- [ ] Subtitle appears below, muted gray
- [ ] All three stagger in smoothly on first scroll-into-view

**Step 3: Verify the sticky tab bar**

- [ ] Tab bar sticks to the top of the viewport when scrolling within the section
- [ ] Active tab has solid `bg-primary` pill — the pill slides between tabs when clicking (spring animation)
- [ ] Each tab has an icon + label
- [ ] On narrow viewport (< 640px): tabs scroll horizontally, no wrap, no visible scrollbar

**Step 4: Verify tab switching**

Click each of the 5 tabs in order:
- [ ] **Quotes** → panel fades up from `y:12` to `y:0`, left copy shows Quotes data, right shows browser frame with video
- [ ] **Orders** → panel cross-fades smoothly, new content loads
- [ ] **Scheduler** → same
- [ ] **Pricing** → same
- [ ] **Analytics** → highlights checklist appears below the solution box

For each panel:
- [ ] Icon badge (square, `bg-primary/10`) renders
- [ ] Feature title is large (`text-3xl md:text-4xl`)
- [ ] THE FRICTION box: rose background, rose eyebrow label
- [ ] THE FIX box: blue background, primary eyebrow label
- [ ] "Watch Full Demo" ghost button visible — clicking it opens VideoModal

**Step 5: Verify browser frame**

- [ ] Frame has rounded corners, subtle border, large shadow
- [ ] Title bar: red/yellow/green traffic light dots, centered "Shop Titan" label
- [ ] Video area is `aspect-video` (16:9) with dark fallback background
- [ ] VideoPlayer renders (may show nothing if video files don't exist locally — that's expected)

**Step 6: Verify Tier 2 grid**

Scroll below the tab panel:
- [ ] "MORE FEATURES" eyebrow pill appears
- [ ] "Additional Capabilities" heading
- [ ] 6 cards in a 3-column grid (desktop), 2-col (tablet), 1-col (mobile)
- [ ] Each card: icon badge, title, 2-line summary (line-clamp)
- [ ] Hover: border tints primary blue, icon badge lightens
- [ ] Cards stagger in with `whileInView` cascade

**Step 7: Commit (if any visual fixes were needed)**

If you made any minor visual tweaks during verification:
```bash
git add apps/marketing/components/sections/FeaturesSection.tsx
git commit -m "fix: features section - visual polish after review"
```
...
- `scrollbar-none` is defined in `apps/marketing/app/globals.css` as `.scrollbar-none::-webkit-scrollbar { display: none }` — already in the project
- `px-mobile` is a custom Tailwind utility defined in the project for horizontal padding
- The `FeatureGrid`, `FeatureGridCard`, and `cn` imports are removed — those files still exist in `apps/marketing/components/ui/` but are no longer used by this component. Do not delete them (they may be used elsewhere)
- `layoutId="activeFeatureTab"` requires `LayoutGroup` wrapper to work correctly when multiple layout animations exist on the page simultaneously
- `AnimatePresence mode="wait"` ensures the exit animation completes before the enter animation starts — critical for the clean tab swap feel
- The old `FeatureBlock` sub-component (formerly lines 530–688) is gone — it no longer exists in the new file. The new component is self-contained
- All 5 Tier 1 features have `videoSrc` set to `/videos/feature-*.mp4` — these files may not exist in local dev. VideoPlayer will show a dark fallback area. This is expected behavior
